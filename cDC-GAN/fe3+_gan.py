...
for m in range(200):
    ...
    for t_p in time_point:
        ...
        # k_std = (k_data-np.mean(k_data))/np.std(k_data)   # bioazhunhua
        with open('./data/ion_data/'+t_p+'d/no_'+str(m)+'_'+t_p+'天化学结果_Fe3.pkl', 'rb') as f:
...
for epoch in range(opt.n_epochs):
    for idx, imgs in enumerate(x_train):
        ...
        if (epoch+1) % 20 == 0 and idx == 199:
                ...
                plt.savefig('./trained_model/fe3+/fake1_' + str(epoch) + '.png')
                plt.imshow(torch.Tensor.detach(gen_imgs[155, 0, :, :]).cpu().numpy())  # my
                plt.savefig('F:/inversion_para/final_ions_gan/fe3+/fake155_' + str(epoch) +'.png')
                torch.save(generator.state_dict(), './trained_model/fe3+/Generator_{}model.pt'.format(epoch))
