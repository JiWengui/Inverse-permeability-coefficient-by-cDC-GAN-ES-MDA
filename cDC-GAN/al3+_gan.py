...
# for p in time_point:
#     os.makedirs('F:/inversion_para/final_ions_gan/al3+/' + p + 'd', exist_ok=True)
...
for m in range(200):
    ...
    for t_p in time_point:
        ...
        with open('./data/ion_data/'+t_p+'d/no_'+str(m)+'_'+t_p+'天化学结果_Al.pkl', 'rb') as f:
...
for epoch in range(opt.n_epochs):
    for idx, imgs in enumerate(x_train):
        ...
        if (epoch+1) % 20 == 0 and idx == 99:
                plt.imshow(torch.Tensor.detach(gen_imgs[1, 0, :, :]).cpu().numpy())  # 5 d
                plt.savefig('./trained_model/al/fake1_' + str(epoch) + '.png')
                plt.imshow(torch.Tensor.detach(gen_imgs[155, 0, :, :]).cpu().numpy())  # 650 d
                plt.savefig('./trained_model/al/fake155_' + str(epoch) +'.png')
                torch.save(generator.state_dict(), './trained_model/al/Generator_{}model.pt'.format(epoch))  # save_model
