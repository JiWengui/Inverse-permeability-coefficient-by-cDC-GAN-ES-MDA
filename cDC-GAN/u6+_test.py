...
modelGenerator.cuda()
modelGenerator.load_state_dict(torch.load('./trained_model/u/Generator_99model.pt'))  #219
modelGenerator.eval()
...
for i in range(10):
    ...
    for num, tt in enumerate(time_point):
        ...
        plt.savefig('./test_model/u6+_test/' + tt + 'd/' + str(i) + '.png')
        plt.imshow(x_true[i][num],origin='lower')
        plt.savefig('./test_model/u6+_test/'+tt+'d/真' + str(i) + '.png')
        # save test results
        with open('./test_model/u6+_test/'+tt+'d/real_ion{}.plk'.format(i), 'wb') as f:
                pickle.dump(x_true[i][num], f)
        with open('./test_model/u6+_test/'+tt+'d/fake_ion{}.plk'.format(i), 'wb') as f:
                pickle.dump(fake, f)
