...
# for p in time_point:  # Creating a save location for validation results
#     os.makedirs('F:/inversion_para/al3+_test/' + p + 'd', exist_ok=True)
...
modelGenerator.load_state_dict(torch.load('./trained_model/al/Generator_99model.pt'))
modelGenerator.eval()
...
for m in range(10):
    ...
    for t_p in time_point:
        ...
        with open('./ion_data/'+t_p+'d/no_'+str(m+star_num)+'_'+t_p+'天化学结果_Al.pkl', 'rb') as f:
...
for i in range(10):
    ...
    for num, tt in enumerate(time_point):
        ...
        plt.savefig('./test_model/al3+_test/' + tt + 'd/' + str(i) + '.png') 
        plt.imshow(x_true[i][num],origin='lower')
        plt.savefig('./test_model/al3+_test/'+tt+'d/真' + str(i) + '.png')
        # save test results
        with open('./test_model/al3+_test/'+tt+'d/real_ion{}.plk'.format(i), 'wb') as f:
                pickle.dump(x_true[i][num], f)  # 保存真的铀浓度数据
        with open('./test_model/al3+_test/'+tt+'d/fake_ion{}.plk'.format(i), 'wb') as f:
                pickle.dump(fake, f)
