...
# for p in time_point:
#     os.makedirs('F:/inversion_para/final_ions_gan/ca2+/' + p + 'd', exist_ok=True)
...
for m in range(200):
    ...
    for t_p in time_point:
        ...
        with open('./data/ion_data/'+t_p+'d/no_'+str(m)+'_'+t_p+'天化学结果_Ca.pkl', 'rb') as f:
...
for epoch in range(opt.n_epochs):
    for idx, imgs in enumerate(x_train):
        ...
        if (epoch+1) % 20 == 0 and idx == 199:
                ...
                plt.savefig('./trained_model/ca2+/fake1_' + str(epoch) + '.png')
                plt.imshow(torch.Tensor.detach(gen_imgs[155, 0, :, :]).cpu().numpy())
                plt.savefig('./trained_model/ca2+/fake155_' + str(epoch) + '.png')
                plt.close()
                torch.save(generator.state_dict(), './trained_model/ca2+/Generator_{}model.pt'.format(epoch))